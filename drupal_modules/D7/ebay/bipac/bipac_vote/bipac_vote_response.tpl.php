    ...
    <div class="vote-content">
      <ul>
        <?php if (!empty($data['RegistrationNote'])): ?>
          <li><?php echo $data['RegistrationNote']; ?></li>
        <?php endif; ?>
      </ul>
    </div>
  </div>
  ...
  <div class="vote-accordion vote-tab" id="vote-early">
    ...
    <ul class="vote-content">
      <?php if (!empty($dates['EarlyBegin'])): ?>
        <li><em>General Early Vote Dates:</em>
        <p>Early Voting Begins: <?php echo $dates['EarlyBegin']; ?></p>
        <?php if (!empty($dates['EarlyEnd'])): ?>
          <p>Early Voting Ends: <?php echo $dates['EarlyEnd']; ?></p>
        <?php endif; ?>
        <li>
      <?php endif; ?>
      <?php if (!empty($data['EarlyVoteNote'])): ?>
        <li><em>Vote Early in Person:</em> <?php echo $data['EarlyVoteNote']; ?></li>
      <?php endif; ?>
      <?php if (!empty($data['EarlyVoteFormURL'])): ?>
        <li><em>Online Early Vote Resources:</em> <?php echo $state; ?> has <a href="<?php echo $data['EarlyVoteFormURL']; ?>" rel="external">early vote forms</a> available online.</li>
      <?php endif; ?>
    </ul>
  </div>
  ...
  <div class="vote-accordion vote-offices">
    ...
    <ul class="vote-content">
      <?php if (!empty($data['reg_address'])): ?>
        ...
        <h5>Local</h5>
        <?php if (!empty($data['reg_address'])): ?>
          <p><?php echo bipac_checkurl($data['reg_address'], 'link to registration information', 'Link to Local Information'); ?></p>
        <?php endif; ?>
        ...
        <?php if (!empty($data['reg_phone'])): ?>
          <p>Phone: <?php echo bipac_format_phone($data['reg_phone']); ?></p>
        <?php endif; ?>
        </li>
      <?php endif; ?>
        ...
        <ul>
          <li>
            ...
            <?php if (!empty($data['abs_phone'])): ?>
              <p><?php echo bipac_format_phone($data['abs_phone']); ?></p>
            <?php endif; ?>
            <?php if (!empty($data['abs_fax'])): ?>
              <p><?php echo bipac_format_phone($data['abs_fax']); ?></p>
            <?php endif; ?>
          </li>
        </ul>
      <?php endif; ?>
