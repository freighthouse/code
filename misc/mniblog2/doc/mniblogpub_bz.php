...
//added jc
include 'sites/all/modules/acl/acl.module';
include 'sites/all/modules/cck/content.module';
...
function get_first_paragraph($text){
	/*
		get first paragraph of body for an unpaid teaser -
		unless it's a byline in which case get paragraph after that as well
	...
	*/
	...
	//$text; //no paragraphs, return nothing -
	// not much point in having it be alacarte if it ever returns whole article
	if(!$t[0]){
		return "";
	}
...
function get_type($taxonomy){
	...
	if it has any of mainwire,fibullets,fxbullets,credit,cobullets,eqbullets,
	fxpackage,fipackage then its subscription_article
	otherwise free_article

	note that this version only returns first match, assuming multiple passes
	of this script (or different versions of it) on multiple time delayed
	versions of stories.  rewrite if this script needs to process articles
	into all matching types
	*/
	if(in_array('alacarte',$taxonomy)){
		return 'alacarte_article';
	}
	if(array_intersect(array('mainwire','fibullets','fxbullets','credit',
	'cobullets','eqbullets','fxpackage','fipackage'),$taxonomy)){
...
function newstory_alacarte($head, $body, $taxonomy)
{
	...
	$pnode = array(
		...
		'format'  => 1,
		'body'    => "",
	// TODO set price in price handler or a setting that can be edited
	// administratively
    'sell_price' => 5,
    'sku'    => "alacarte",
    'model'    => "alacarte",
	...
	// associate product with article in ubercart
...
function process_files(){
	...
	if ($handle = opendir($path_to_xml_dirs."delivery"))
	{
		while (false !== ($file = readdir($handle)))
		{
			...
			// TODO MNI please note - i don't know how you want to do this -
			// if there will be multiple copies of this script, or one copy
			// running against multiple time delayed copies of the articles...
			// this is set up to work on a test directory so please edit
			// appropriately
			$type=get_type($codes);
