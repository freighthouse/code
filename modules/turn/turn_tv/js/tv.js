...
OO.ready(function() {
	turntvplayer = OO.Player.create('tvid', turntvids[turntvon], {
		width: 1376,
		height: 774,
		wmode: 'opaque',
		onCreate: function(player) {
				...
				*/
				player.mb.subscribe('played', 'turntv', function() {
				...
				player.mb.subscribe('playbackReady', 'turntv', function() {
					turntvplayer.play();
...
function tv_qps() {
	jQuery.getJSON('/sites/all/modules/turn_home_js/qps.php', function(data) {
		var items = [];
...
function tv_stat() {
	...
	jQuery.getJSON('/sites/all/modules/turn_tv/stats.php', function(data) {
...
function tv_tweets() {
	...
	jQuery('#tweetshere').load('/tweets .view-content', function() {
		tv_twi = -1;
		tv_twflip();
	});
}
...
function tv_tix() {
	jQuery.getJSON('/sites/all/modules/turn_tv/js/tick-sample.json', function(data) {
